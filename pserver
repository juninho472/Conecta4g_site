...
cd /var/www/html
wget https://worldofdragon.net/painel/painelv5/PainelV5.zip > /dev/null 2>&1
unzip PainelV5.zip > /dev/null 2>&1
rm -rf PainelV5.zip index.html > /dev/null 2>&1
service apache2 restart > /dev/null 2>&1
sleep 1
...
sleep 1
cd
wget https://worldofdragon.net/painel/painelv5/ssh.sql > /dev/null 2>&1
sleep 1
if [[ -e "$HOME/ssh.sql" ]]; then
    mysql -h localhost -u root -p$senha --default_character_set utf8 ssh < ssh.sql
    rm /root/ssh.sql
else
    clear
